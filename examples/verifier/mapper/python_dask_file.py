...
def sum_array():
    ...
    i=0
    start = time.time()
    numbers_bag_0 = daskbag.read_text('input_data_0.csv',blocksize=blocksize).map(json.loads)
    
   
    start1 = time.time()
    #print("Time taken to execute the code: " + str(start1 - start))
...
def run_prog():
	output = sum_array()
	print(output[:4])
	writeTofile(output_file_dask,output)
